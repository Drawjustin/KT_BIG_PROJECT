...
// const API_SERVER_HOST = `https://api.officialsos.shop/minwon`;
...
export const loginPost = async (loginParam) => {
  ...
  const requestBody = {
    memberEmail: loginParam.memberEmail,
    memberPassword: loginParam.memberPassword
  };
  ...
  try {
    ...
    // return res.data;
    // 응답 데이터의 userEmail을 memberEmail로 변경 
    console.log("Login API Response:", res.data); // 응답 구조 확인
    return res.data;
  } catch (error) {
...
export const logoutPost = async (logoutParam) => {
  try {
    const res = await jwtAxios.post(`${host}/logout`, {
      memberEmail: logoutParam.memberEmail,
